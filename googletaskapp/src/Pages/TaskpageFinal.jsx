...
function TestimonialCard(props) {
    ...
    const { title, status, index, _id } = props;
    return (
        <Flex
            boxShadow={'lg'}
            maxW={'640px'}
            ...
            <Flex
                ...
                justifyContent={'space-between'}>
                <HStack>
                    <chakra.p w={'134px'} fontFamily={'Work Sans'} fontWeight={'bold'} fontSize={14} >
                        <chakra.span
                            fontFamily={'Inter'}
                            fontWeight={'medium'}
                            color={'gray.500'}>
                ...
                <Button bg={status ? "whatsapp.100" : "red.100"} onClick={() => handleEdit(_id, status)}>{status ? "Completed" : "Incompleted"}</Button>
                <Button colorScheme='red' rightIcon={<DeleteIcon />} onClick={() => handleDelete(_id)}>Delete</Button>
...
export default function TaskpageFinal() {
        ...
        <Flex
            ...
            <SimpleGrid
                ...
                {loading ?
                    ...
                    : error ? <Errorpage /> : task.map((cardInfo, index) => (
